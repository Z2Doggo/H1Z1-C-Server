...
OUTPUT_NAME="linux_login_server_x86_x64"
SOURCES="../src/linux_login_server.c"

DEFINES_SHARED=""
DEFINES="$DEFINES_SHARED"
DEFINES_D="$DEFINES_SHARED -DYOTE_INTERNAL=1 -DYOTE_SLOW=1"
...
FLAGS_COMPILE_D="$FLAGS_COMPILE_SHARED -O0 -g"
...
if [ ! -d "build_login_server_linux_x86_x64" ]; then
  mkdir build_login_server_linux_x86_x64
fi
pushd build_login_server_linux_x86_x64
...
gcc $FLAGS_COMPILE_D -o login_server_module.so ../src/h1z1_game_server.c $DEFINES_D -shared $FLAGS_LINK_D $LIBS_D
rm .reload-lock

gcc $FLAGS_COMPILE_D -o login_server ../src/linux_login_server.c $DEFINES_D $FLAGS_LINK_D $LIBS_D
