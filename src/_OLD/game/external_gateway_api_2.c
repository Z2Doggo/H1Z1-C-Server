...
#define GATEWAY_PACKET_KINDS \
	GATEWAY_PACKET_KIND(Gateway_Packet_Kind_Unhandled, L"Unhandled"), \
	GATEWAY_PACKET_KIND(Gateway_Packet_Kind_LoginRequest, L"LoginRequest"), \
	GATEWAY_PACKET_KIND(Gateway_Packet_Kind_LoginReply, L"LoginReply"), \
	GATEWAY_PACKET_KIND(Gateway_Packet_Kind_TunnelPacketToExternalConnection, L"TunnelPacketToExternalConnection"), \
	GATEWAY_PACKET_KIND(Gateway_Packet_Kind_TunnelPacketFromExternalConnection, L"TunnelPacketFromExternalConnection"), \
	GATEWAY_PACKET_KIND(Gateway_Packet_Kind_ChannelIsRoutable, L"ChannelIsRoutable"), \
	GATEWAY_PACKET_KIND(Gateway_Packet_Kind__End, L"")
...
wchar_t* gateway_packet_names[Gateway_Packet_Kind__End + 1] =
{
                                 ...
                                 u8* buffer)
{
	...
	switch (packet_kind)
	{
		...
		default:
		{
			printf("[!] Packing %ws not implemented\n", gateway_packet_names[packet_kind]);
		}
	}
	return offset;
}
                                    ...
                                    Arena* arena)
{
	...
	switch (packet_kind)
	{
		case Gateway_Packet_Kind_LoginRequest:
		{
			...
			offset += sizeof(u32);
			packet->server_ticket = memory_arena_allocate(arena, packet->server_ticket_length);
			...
			offset += sizeof(u32);
			packet->client_protocol = memory_arena_allocate(arena, packet->client_protocol_length);
			...
			offset += sizeof(u32);
			packet->client_build = memory_arena_allocate(arena, packet->client_build_length);
		...
		default:
		{
			printf("[!] Unpacking %ws not implemented\n", gateway_packet_names[packet_kind]);
		}
	}
}
                                       ...
                                       u32 total_length)
{
	...
	if (session_state->connection_args.should_dump_tunnel)
	{
		...
		_snwprintf_s(dump_path, 256, 256, L"packets\\%llu_%llu_S_tunneldata_%u.bin", global_tick_count, global_packet_dump_count++, tunnel_packet.channel);
                                  ...
                                  void* packet_ptr)
{
	...
	{
		wchar_t dump_path[256] = { 0 };
		_snwprintf_s(dump_path, 256, 256, L"packets\\%llu_%llu_S_gateway_%ws.bin", global_tick_count, global_packet_dump_count++, gateway_packet_names[packet_kind]);
																		...
																		u32 data_length)
{
	...
	switch (packet_id)
	{
		case GATEWAY_LOGINREQUEST_ID:
		{
			...
			printf("[*] (%u) Handling %ws...\n", channel, gateway_packet_names[packet_kind]);


			Gateway_Packet_LoginRequest packet = { 0 };
			gateway_packet_unpack(data, data_length, packet_kind, &packet, &server_state->arena_per_frame);

			printf("[*] Enabling encryption for session\n");
			// TODO(rhett): what is this flag for when streams have their own?
			session_state->connection_args.use_encryption = TRUE;
			...
			gateway_packet_send(server_state, session_state, &server_state->arena_per_frame, 32, Gateway_Packet_Kind_LoginReply, &login_reply);
			...
			gateway_packet_send(server_state, session_state, &server_state->arena_per_frame, 32, Gateway_Packet_Kind_ChannelIsRoutable, &channel_is_routable_0);
			...
			gateway_packet_send(server_state, session_state, &server_state->arena_per_frame, 32, Gateway_Packet_Kind_ChannelIsRoutable, &channel_is_routable_1);
		...
		case GATEWAY_TUNNELPACKETFROMEXTERNALCONNECTION_ID:
		{
			...
			gateway_packet_unpack(data, data_length, Gateway_Packet_Kind_TunnelPacketFromExternalConnection, &tunnel_data, &server_state->arena_per_frame);

			if (session_state->connection_args.should_dump_tunnel)
			{
				wchar_t dump_path[256] = { 0 };
				_snwprintf_s(dump_path, 256, 256, L"packets\\%llu_%llu_C_tunneldata_%u.bin", global_tick_count, global_packet_dump_count++, tunnel_data.channel);
	...
	{
		wchar_t dump_path[256] = { 0 };
		_snwprintf_s(dump_path, 256, 256, L"packets\\%llu_%llu_C_gateway_%u_%ws.bin", global_tick_count, global_packet_dump_count++, channel, gateway_packet_names[packet_kind]);
		server_state->platform_api->buffer_write_to_file(dump_path, data, data_length);
	}
}