...
internal u32 gateway_packet_pack(
...
internal void gateway_tunnel_data_send(
...
)
{
	...
	u32 packed_length = gateway_packet_pack(Gateway_Packet_Kind_TunnelPacketToExternalConnection, &tunnel_packet, base_buffer);


	output_stream_write(&session_state->output_stream, base_buffer, packed_length, FALSE, server_state, session_state);
}

internal void gateway_packet_send(
	Server_State* server_state,
	Session_State* session_state,
	Memory_Arena* arena,
	u32 max_length,
...
)
{
	...
	u32 packed_length = gateway_packet_pack(packet_kind, packet_ptr, packed_buffer);



	output_stream_write(&session_state->output_stream, packed_buffer, packed_length, FALSE, server_state, session_state);
}

internal void gateway_packet_handle(
	Server_State* server_state,
	Session_State* session_state,
	u8* data,
	u32 data_length
)
{
	...
	switch (packet_id)
	{
		case GATEWAY_LOGINREQUEST_ID:
		{
			...
			printf("[Gateway] Enabling encryption for session\n");
